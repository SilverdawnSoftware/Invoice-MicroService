...
{
  	public partial class InvoiceChangeStatusCommand : Orleans.Grain , IInvoiceChangeStatusCommand
    {
        ...
        /// </remarks>
      public async Task<IInvoiceView> InvoiceChangeStatus(IInvoiceChangeStatus invoiceChangeStatus)
      {
			try
          {
              ...
              {
                   if (db.Invoices.Any(w =>  w.InvoiceId == invoiceChangeStatus.InvoiceId))
                   {
                       var invoice = db.Invoices.First(w =>  w.InvoiceId == invoiceChangeStatus.InvoiceId);
                    invoice.Status=(Database.InvoiceStatus)invoiceChangeStatus.Status;  	
